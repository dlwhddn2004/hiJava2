...
/*
 	...
 	프로그램 소스코드 안에 다른 프로그램을 위한 정보를 미리 약속된 형식으로 포함시킨것(JDK.15 부터 지원함)
 	...
 	2. ()안에 매개변수를 선언할 수 없다.
 	3. 예외를선언할 수 없다.
 	4.요소의 타입에 타입 매개변수(지네릭타입문자)를 사용 할 수 없다.
 */
...
public @interface PrintAnnotation {
	...
	String value() default "-"; //기본값을 '-'로 지정 ex) 타입요소를 value로 잡고 String 타입으로 정의함, 기본값은 - 이다 라는느낌?
	int count() default 20; 	//기본값을20으로 지정
}
