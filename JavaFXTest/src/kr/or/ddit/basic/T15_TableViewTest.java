...
public class T15_TableViewTest extends Application {
	  @Override
	   public void start(Stage primaryStage) throws Exception {
	      ...
	      // 해당 컬럼에 나타낼 데이터 연결하기 (만든 맴버클래스의 이름에 맞게)
	      ...
	      // 컬럼 병합하기 nameCol에 관리하는 컬럼을 넣는다(병합)
	      nameCol.getColumns().addAll(kornameCol,engNameCol);
	      
	      //테이블컬럼을 정할때 제너릭에 사용할 타입을 선정
	      ...
	      table.setItems(data); //맨첨에 생성자에서 세팅해도됨 
	      //------------------------------------------------------------------------------
	      ...
	      btnAdd.setOnAction(e->{
	    	  if(txtKorName.getText().isEmpty() //isEmpty 뜻 비어있는지
	    	  ...
	    	  // ObservableList라 테이블이 자동으로 바뀌면 똑같이 바뀌어진다.
	    	  ...
	    	  infoMsg("작업결과", txtKorName.getText()
	    			  +" 님 정보를 추가했습니다");
	      ...
	      btnEdit.setOnAction(e->{
	    	  ...
	    	  data.set( //list 넣는것처럼 (index,수정할것)
	    			  table.getSelectionModel().getSelectedIndex(), //테이블의. 현재선택되어있는 모델의. 인덱스
	      ...
	      btnDel.setOnAction(e ->{
	    	  if(table.getSelectionModel().isEmpty()) { //getSelecttionModel() ! = -1
	    		  errMsg("작업오류", "삭제할 자료를 선택 한 후 삭제하세요");
	    		  return;
	    	  }
