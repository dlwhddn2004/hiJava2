...
public class T13_RadioButtonTest extends Application{
	...
	public void start(Stage primaryStage) throws Exception {
		...
		RadioButton rb1 = new RadioButton("HOME"); // rb1.setText("HOME");
		rb1.setToggleGroup(group); //라디오 버튼을 그룹에 추가
		rb1.setUserData("Home"); // 선택했을때의 값을 나타내기위한 데이터
		...
		//그룹 내에서 RadioButton 중 하나가 선택되었을때 처리하기 <?> -> Toggle로 바꿔줌
		group.selectedToggleProperty().addListener(new ChangeListener<Toggle>() {

			@Override
			public void changed(ObservableValue<? extends 
					Toggle> observable, Toggle oldValue, Toggle newValue) {
				//선택한 항복(라디오버튼)이 있는지 검사 , 근데 메소드에 있는 newValue로 바로 가져와도됨
				//if(newValue.getUserData !=null) 같은의미
				if(group.getSelectedToggle().getUserData() != null) {
					String url = group. ////url에  userdata Home,Calendar,Contacts 넣는다.
		...
		Scene scene = new Scene(hbox,250,150); //높이와 넓이
